...
class TotalsReportBloc extends Bloc<TotalsReportEvent, TotalsReportState> {
  ...
  Stream<TotalsReportState> mapEventToState(
    ...
    if (event is TotalsReportInitialEvent) {
      ...
      if ((event.printFromBatch != null) && (event.printFromBatch == true)) {
        yield TotalsReportPrinting();
      }

      report.printTotalsReport(await getTotalsData(), onPrintOK, onPrintError);
    } else if (event is TotalsReportOnPrintOKEvent) {
      if (printFromBatch)
        yield TotalsReportPrintOk(printFromBatch);
      else
  ...
  void onPrintError(int type) {
    this.add(TotalsReportOnPrintErrorEvent());
  }

  Future<List<Map<String, dynamic>>> getTotalsData() async {
